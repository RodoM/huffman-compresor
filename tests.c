...
void test_codificar_valores(){
  ...
  hoja1->valor = 0; hoja1->frecuencia = 2;
  hoja2->valor = 1; hoja2->frecuencia = 5;
  hoja3->valor = 2; hoja3->frecuencia = 1;
  hoja4->valor = 3; hoja4->frecuencia = 0;
  ...
  codificar_valores(arbol, codif, valoresSerializados, &cantHojas, &codificacionLen, codificaciones);
...
void test_serializar_arbol(){
  ...
  char *codificado = malloc(sizeof(char)* 7);
...
int btree_comp(BTree arbol1, BTree arbol2){
  int res = 0;
  if(arbol1 == NULL && arbol2 == NULL) return 0;
  if(arbol1 != NULL && arbol2 == NULL) return 1;
  if(arbol1 == NULL && arbol2 != NULL) return 1;
  if(arbol1 == NULL && arbol2 == NULL){
    if(arbol1->valor != arbol2->valor) return 1;
...
void test_decodificar_archivo(){
  ...
  hoja1->der = hoja1->izq = hoja2->der = hoja2->izq = NULL;
  hoja3->der = hoja3->izq = hoja4->der = hoja4->izq = NULL;
  hoja1->valor = 108; hoja1->frecuencia = 0;
  hoja2->valor = 97; hoja2->frecuencia = 0;
  hoja3->valor = 111; hoja3->frecuencia = 0;
  hoja4->valor = 72; hoja4->frecuencia = 0;
  BTree izq = btree_unir(-1, hoja4, hoja3);
  BTree der = btree_unir(-1, hoja1, hoja2);
  BTree arbol = btree_unir(-1, izq, der);
  ...
  codificacion = "00011011";
  int decodlen = 0;

  char *texto_decodificado = decodificar_archivo(arbol, 8, codificacion, &decodlen);
  texto_decodificado[decodlen] = '\0';
  char *texto = malloc(sizeof(char)*4);
  texto = "Hola";

  assert(strcmp(texto_decodificado, texto)==0);
  assert(decodlen==4);
}
